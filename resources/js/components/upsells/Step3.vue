...
export default {
  ...
  computed: {
    ...
    selectList() {
      // (Main product amount + Upsell amount) - upsells select negth
      const data = Array(Number(this.selectedProductData.quantity || this.selectedProductData.deal || 1) + this.selectedProductUpsellsQuantity).slice(0, this.upsellPrices && this.upsellPrices.length).fill('').map((item, index) => {
        const value = index + 1
